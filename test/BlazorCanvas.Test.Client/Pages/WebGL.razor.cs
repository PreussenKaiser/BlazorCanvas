...
public partial class WebGL
{
    ...
    {
        this.context = await this.canvas.CreateWebGLAsync(new WebGLContextAttributes
        {
            PowerPreference = WebGLContextAttributes.POWER_PREFERENCE_HIGH_PERFORMANCE
        });

        await this.context.ClearColorAsync(0, 0, 0, 1);
        await this.context.ClearAsync(BufferBits.COLOR_BUFFER_BIT);

        var program = await this.InitProgramAsync(this.context, VS_SOURCE, FS_SOURCE);

        var vertexBuffer = await this.context.CreateBufferAsync();
        await this.context.BindBufferAsync(BufferType.ARRAY_BUFFER, vertexBuffer);
        ...
        await this.context.BufferDataAsync(BufferType.ARRAY_BUFFER, vertices, BufferUsageHint.STATIC_DRAW);
