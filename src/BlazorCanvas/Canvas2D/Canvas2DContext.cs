...
namespace BlazorCanvas.Canvas2D;
...
public class Canvas2DContext : RenderingContext
{
    ...
    public Canvas2DContext(BECanvas reference)
        : base(reference, CONTEXT_NAME)
    {
    }
    ...
    public async Task<object> CreatePatternAsync(ElementReference image, RepeatPattern repeat) => await this.CallMethodAsync<object>(CREATE_PATTERN_METHOD, image, this.repeatNames[(int)repeat]);
