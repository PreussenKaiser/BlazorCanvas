...
public class WebGLContext : RenderingContext
{
    ...
    /// <summary>
    /// Initializes <see cref="WebGLContext"/>.
    /// </summary>
    /// <param name="reference"></param>
    /// <param name="attributes"></param>
    public WebGLContext(BECanvas reference, WebGLContextAttributes attributes = null)
        : base(reference, CONTEXT_NAME, attributes)
    {
    }
    ...
    public async Task ClearColorAsync(float red, float green, float blue, float alpha)
        => await this.BatchCallAsync(CLEAR_COLOR, isMethodCall: true, red, green, blue, alpha);

    public async Task ClearAsync(BufferBits mask)
        => await this.BatchCallAsync(CLEAR, isMethodCall: true, mask);

    public async Task<WebGLContextAttributes> GetContextAttributesAsync()
        => await this.CallMethodAsync<WebGLContextAttributes>(GET_CONTEXT_ATTRIBUTES);

    public async Task<bool> IsContextLostAsync()
        => await this.CallMethodAsync<bool>(IS_CONTEXT_LOST);

    public async Task ScissorAsync(int x, int y, int width, int height)
        => await this.BatchCallAsync(SCISSOR, isMethodCall: true, x, y, width, height);

    public async Task ViewportAsync(int x, int y, int width, int height)
        => await this.BatchCallAsync(VIEWPORT, isMethodCall: true, x, y, width, height);

    public async Task ActiveTextureAsync(Texture texture)
        => await this.BatchCallAsync(ACTIVE_TEXTURE, isMethodCall: true, texture);

    public async Task BlendColorAsync(float red, float green, float blue, float alpha)
        => await this.BatchCallAsync(BLEND_COLOR, isMethodCall: true, red, green, blue, alpha);

    public async Task BlendEquationAsync(BlendingEquation equation)
        => await this.BatchCallAsync(BLEND_EQUATION, isMethodCall: true, equation);

    public async Task BlendEquationSeparateAsync(BlendingEquation modeRGB, BlendingEquation modeAlpha)
        => await this.BatchCallAsync(BLEND_EQUATION_SEPARATE, isMethodCall: true, modeRGB, modeAlpha);

    public async Task BlendFuncAsync(BlendingMode sfactor, BlendingMode dfactor)
        => await this.BatchCallAsync(BLEND_FUNC, isMethodCall: true, sfactor, dfactor);

    public async Task BlendFuncSeparateAsync(
        BlendingMode srcRGB, BlendingMode dstRGB,
        BlendingMode srcAlpha, BlendingMode dstAlpha)
        => await this.BatchCallAsync(BLEND_FUNC_SEPARATE, isMethodCall: true, srcRGB, dstRGB, srcAlpha, dstAlpha);

    public async Task ClearDepthAsync(float depth)
        => await this.BatchCallAsync(CLEAR_DEPTH, isMethodCall: true, depth);

    public async Task ClearStencilAsync(int stencil)
        => await this.BatchCallAsync(CLEAR_STENCIL, isMethodCall: true, stencil);

    public async Task ColorMaskAsync(bool red, bool green, bool blue, bool alpha)
        => await this.BatchCallAsync(COLOR_MASK, isMethodCall: true, red, green, blue, alpha);

    public async Task CullFaceAsync(Face mode)
        => await this.BatchCallAsync(CULL_FACE, isMethodCall: true, mode);

    public async Task DepthFuncAsync(CompareFunction func)
        => await this.BatchCallAsync(DEPTH_FUNC, isMethodCall: true, func);

    public async Task DepthMaskAsync(bool flag)
        => await this.BatchCallAsync(DEPTH_MASK, isMethodCall: true, flag);

    public async Task DepthRangeAsync(float zNear, float zFar)
        => await this.BatchCallAsync(DEPTH_RANGE, isMethodCall: true, zNear, zFar);

    public async Task DisableAsync(EnableCap cap)
        => await this.BatchCallAsync(DISABLE, isMethodCall: true, cap);

    public async Task EnableAsync(EnableCap cap)
        => await this.BatchCallAsync(ENABLE, isMethodCall: true, cap);

    public async Task FrontFaceAsync(FrontFaceDirection mode)
        => await this.BatchCallAsync(FRONT_FACE, isMethodCall: true, mode);

    public async Task<T> GetParameterAsync<T>(Parameter parameter)
        => await this.CallMethodAsync<T>(GET_PARAMETER, parameter);

    public async Task<Error> GetErrorAsync()
        => await this.CallMethodAsync<Error>(GET_ERROR);

    public async Task HintAsync(HintTarget target, HintMode mode)
        => await this.BatchCallAsync(HINT, isMethodCall: true, target, mode);

    public async Task<bool> IsEnabledAsync(EnableCap cap)
        => await this.CallMethodAsync<bool>(IS_ENABLED, cap);

    public async Task LineWidthAsync(float width)
        => await this.CallMethodAsync<object>(LINE_WIDTH, width);

    public async Task<bool> PixelStoreIAsync(PixelStorageMode pname, int param)
        => await this.CallMethodAsync<bool>(PIXEL_STORE_I, pname, param);

    public async Task PolygonOffsetAsync(float factor, float units)
        => await this.BatchCallAsync(POLYGON_OFFSET, isMethodCall: true, factor, units);

    public async Task SampleCoverageAsync(float value, bool invert)
        => await this.BatchCallAsync(SAMPLE_COVERAGE, isMethodCall: true, value, invert);
    ...
    private async Task<int> GetDrawingBufferWidthAsync()
        => await this.GetPropertyAsync<int>(DRAWING_BUFFER_WIDTH);
    private async Task<int> GetDrawingBufferHeightAsync()
        => await this.GetPropertyAsync<int>(DRAWING_BUFFER_HEIGHT);
